...
class BusSubscriber:
    ...
    def __init__(self, callback, db_name=None):
        self._channels = set()
    ...
    def trigger_notification_dispatching(self):
        ...
        if not db_name:
            _logger.error("MCP bus subscriber missing database name")
            return
...
class MCPBusBridgeThread(threading.Thread):
    ...
    def _subscribe_to_bus(self):
        ...
        try:
            ...
            self.bus_wrapper = BusSubscriber(
                self._on_bus_notification, db_name=self.db_name
            )
